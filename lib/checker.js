...
const Checker = {
  ...
  async extractContent(task, settings) {
    ...
    if (method === 'tab') {
      ...
      if (isSpaShell(fetchResult, task.selector)) {
 ...
 * @param {string} selector - 任务使用的 CSS 选择器
 * @returns {boolean}
 */
function isSpaShell(extraction, selector) {
  const text = (extraction.text || '').trim();
  const html = (extraction.html || '').trim();

  // 使用选择器时信任提取结果：选择器可能匹配小元素导致文本很短，
  // 但这不是 SPA 空壳。只有无选择器（监控全页）时才检测 SPA。
  if (selector) {
    return false;
  }
