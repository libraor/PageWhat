...
│   └── options.css
├── themes/
│   ├── theme-system.js        # 主题系统核心（初始化、切换、选择器创建）
...
```javascript
{
  (id, taskId, errorType, errorMessage, url, timestamp);
}
```
...
Popup 页面提供 **"监控当前页面"** 快捷按钮：

1. 点击 Popup 扩展图标
2. 点击 **"🔒 监控当前页面"** 按钮
3. 自动获取当前页面 URL 和标题，创建文本监控任务（默认 5 分钟间隔）

快速启动监控，无需手动填写 URL 和配置。
...
PageWhat 提供 5 种精心设计的视觉风格，可通过 Options 页面设置标签页切换：
...
每种主题都有完整的色彩系统、排版规范、组件样式和响应式设计支持。

## UI 美化

### Options 页面视觉升级

- **渐变头部** — 蓝色渐变背景 + 圆角底部 + 装饰分隔线
- **浮动标签栏** — 卡片式悬浮设计，阴影层次感
- **增强卡片** — 更大圆角（12px）、柔和阴影、悬停上浮动效
- **平滑动画** — Tab 切换淡入动画、悬停 cubic-bezier 缓动
- **响应式优化** — 平板/手机双断点适配，小屏幕工具栏纵向排列

### Popup 页面简化

- **快捷监控** — "监控当前页面" 一键创建任务
- **移除表单** — 简化 Popup 布局，专注快速操作
- **虚线按钮** — 悬停变实线，视觉引导明确
...
- **新增** 主题系统：提供 5 种精心设计的视觉风格（现代简约、复古经典、未来科技、自然清新、商务专业）
- **优化** Options 页面 UI：渐变头部、浮动标签栏、增强卡片、平滑动画、响应式双断点
- **优化** Popup 页面：移除手动添加表单，新增"监控当前页面"快捷按钮
- **优化** README 文档，添加主题系统、UI 美化、Popup 快捷监控相关说明
